...
Com es pot observar hi ha un desplaçament màxim de $\delta_{y, \max} = 1,42 mm$ situat al punt mig de la peça.
...
\begin{figure}[H]
	...
	\label{fig:40_shear}
\end{figure}
Les tensions tangencials màximes es troben als extrems de la peça, s'observa un valor de $\tau_{xy,\max+} = 7.24 MPa$ i $\tau_{xy,\max-} = -7.24 MPa$. També es pot observar que per aquest mallat que no és gaire refinat, no és clar que el valor que prenguin els resultats siguin del tot correctes.
...
\begin{figure}[H]
	...
	\end{tikzpicture}
\end{figure}
	\item Els components del tensor tensió $\sigma_y$, $\sigma_z$ i $\tau_{xy}$ són nuls d'acord amb les hipòtesis de peça prismàtica
	$$
